...
public class DatabaseHelper7 extends SQLiteOpenHelper {
    ...
    public int getCount(){
        ...
        Cursor cursor;
        SQLiteDatabase db = this.getWritableDatabase();
        cursor = db.rawQuery("SELECT COUNT(*) FROM " + TABLE_NAME + " ORDER BY from_module,id", null);
...
//                    }
    ...
    public Cursor getAllDataByModule(String module, String name){
        Cursor cursor = null;
        String finalModule = "";
        if(module.equals("Received Item")){
            finalModule = "Manual Receive Item";
        }else if(module.equals("Transfer Item")){
            finalModule = "System Transfer Item";
        }else if(module.equals("Sales")) {
            finalModule = "Sales";
        }
        if(name.equals("from_module") && !finalModule.trim().isEmpty()){
            System.out.println("by module " + finalModule);
            System.out.println("query " + "SELECT * FROM " + TABLE_NAME + " WHERE " + name +"='" + finalModule + "';");
            SQLiteDatabase db = this.getWritableDatabase();
            cursor = db.rawQuery("SELECT * FROM " + TABLE_NAME + " WHERE " + name +"='" + finalModule + "';", null);
        }else{
            SQLiteDatabase db = this.getWritableDatabase();
